...
$form=preg_replace_callback('~<(input|textarea)\b[^>]*\bid="(cf_[a-z]+)"[^>]*>~i',function($m)use($fields){
...
$form=preg_replace('~<div class="btn" id="submitform">(.*?)</div>~s','<button type="button" class="btn" id="submitform">$1 <span aria-hidden="true">→</span></button>',$form);
$form=str_replace('id="formmessage"','id="formmessage" role="status" aria-live="polite"',$form);
echo $form;
?>
