...
<?php foreach($carouselSolutions as $item): $cover=hg_solution_cover($item); ?><a class="es-footer-card" href="/blog/?riesenie=<?php echo hg_esc($item['slug']); ?>"><div><span><?php echo hg_esc($item['roles'][0]); ?></span><h3><?php echo hg_esc($item['title']); ?></h3><b>Pozrieť riešenie →</b></div><?php if($cover): ?><img src="<?php echo hg_esc($cover); ?>" width="160" height="120" alt="" loading="lazy" decoding="async"><?php else: ?><span class="es-cover-placeholder" aria-hidden="true"><?php echo sprintf('%02d',$item['id']); ?></span><?php endif; ?></a><?php endforeach; ?>
</div><button type="button" class="es-rotation" data-solution-pause aria-pressed="false">Pozastaviť posúvanie</button><a class="es-all-solutions" href="/blog/?tema=problemy">Všetky riešenia z praxe →</a>
</section>
