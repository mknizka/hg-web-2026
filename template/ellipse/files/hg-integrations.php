...
<div class="eh-integration-grid"><?php foreach($integrationGroups as $group=>$items): ?><details><summary><span class="eh-integration-title"><?php echo hg_esc($group); ?></span><span class="eh-integration-toggle" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M5 12h14M12 5v14"/></svg></span><small><?php echo hg_esc(implode(' · ', array_column(array_slice($items,0,3), 'nazov_integracie'))); ?><?php if(count($items)>3) echo ' +'.(count($items)-3); ?></small></summary><ul><?php foreach($items as $item): ?><li><strong><?php echo hg_esc($item['nazov_integracie']); ?></strong><p><?php echo hg_esc($item['popis']); ?></p></li><?php endforeach; ?></ul></details><?php endforeach; ?></div>
</section>
<?php if(count($integrationLogos)): ?><section class="eh-partners"><h3>Vybraní integrátori a partneri</h3><div class="eh-logo-window" data-logo-marquee><div class="eh-logo-track"><?php for($copy=0;$copy<2;$copy++): ?><div class="eh-logo-set"<?php if($copy) echo ' aria-hidden="true"'; ?>><?php foreach($integrationLogos as $logo): if(empty($logo['img']) || !preg_match('~^(?:/(?!/)|https?://)~i',$logo['img'])) continue; ?><img src="<?php echo hg_esc($logo['img']); ?>" alt="<?php echo $copy?'':hg_esc(hg_plain($logo['name'])); ?>" width="400" height="200" loading="eager" decoding="async"><?php endforeach; ?></div><?php endfor; ?></div></div></section><?php endif; ?>
